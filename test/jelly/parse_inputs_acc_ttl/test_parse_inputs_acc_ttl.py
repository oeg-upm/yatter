...
from yatter import translate

pytest.importorskip("pyjelly")
from yatter.cli import parse_inputs, write_results
...
def test_parse_inputs_acc_ttl(tmp_path: Path):
    ...
    args = _mimic_args(input_mapping_path = str(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'jelly_mapping_test_ttl.ttl')),
