...
def test_write_results_roundtrip_to_ttl(tmp_path: Path):
    output_mapping_path = str(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'output_mapping.ttl'))
    ...
    translated_mapping.parse(output_mapping_path, format='turtle')

    expected_mapping = Graph()
    expected_mapping.parse(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'jelly_mapping_test_ttl.ttl'), format='turtle')
