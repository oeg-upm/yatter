...
def parse_inputs():
    ...
    if args.input_mapping_path and args.output_mapping_path:
        if args.input_mapping_path.endswith('.yml') or args.input_mapping_path.endswith('.yaml'):
            ...
            raise Exception("Input mapping do not have a valid extension (.yml, .yaml, .ttl, .rml, or .r2rml)")
        ...
        sys.tracebacklimit = 0
        raise Exception("No input the correct arguments, run python3 -m yarrrml_translator -h to see the help)")
...
def define_args():
    ...
    parser.add_argument("-f", "--format", required=False, help="Mapping input format. Values: R2RML or RML (default)")
    return parser
